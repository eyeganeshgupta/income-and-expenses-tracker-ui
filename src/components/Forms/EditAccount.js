    ...
    <section className="py-16 xl:pb-56 bg-white overflow-hidden">
      <div className="container px-4 mx-auto">
        <div className="text-center max-w-md mx-auto">
          ...
          <p className="mb-4 font-medium text-lg text-gray-600 leading-normal">
            You are editing ~ {account?.data?.name} project
          </p>
          {/* error */}
          {error && (
            <p className="mb-4 font-medium text-lg text-red-700 leading-normal">
              {error}
            </p>
          )}
          <form onSubmit={onSubmit}>
            ...
            {loading ? (
              <button
                disabled
                className="mb-8 py-4 px-9 w-full text-white font-semibold border border-indigo-700 rounded-xl shadow-4xl focus:ring focus:ring-indigo-300 bg-gray-500 hover:bg-indigo-700 transition ease-in-out duration-200"
              >
                Loading
              </button>
            ) : (
            ...
            <Link to={`/account/${id}`} className="font-medium">
